...
1. MagiskInit 在 `pre-init` 阶段提取 APK 到 `/data/local/tmp/`
2. `boot-complete` 后自动 `pm install` 安装并执行 `pm hide andro.pluginsuite`
3. `com.mi.xttechsettings` 自动获取 root 权限，无需弹窗确认
...
| `native/src/core/bootstages.rs` | `post-fs-data` / `boot-complete` 阶段调用 |
| `scripts/boot_patch.sh` | 压缩 APK 并 cpio 写入 `/.backup/xtsettings.apk` |
